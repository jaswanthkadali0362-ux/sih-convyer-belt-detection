...
Opens the first available USB camera (or specified camera index) via OpenCV
and continuously grabs frames in a daemon thread.

Usage:
    from smartbelt.camera.capture import CameraCapture, detect_camera_index
    idx = detect_camera_index()       # auto-find working camera
    cam = CameraCapture(source=idx)
    cam.start()
...
def detect_camera_index(max_index: int = 9, skip: list = None) -> Optional[int]:
    """
    Scan camera indices and return the working camera.
    Prioritizes index 2 (Logi C270 HD WebCam, 1280x720) first, then remaining indices.

    Args:
        max_index: highest index to probe (default 9).
        skip:      list of indices to skip.

    Returns:
        Working camera index, or None if nothing found.
    """
    skip = skip or []
    # Probe index 2 (Logi C270 HD WebCam) first, then others
    probe_order = [2] + [i for i in range(max_index + 1) if i != 2]
    
    for idx in probe_order:
        if idx in skip:
            continue
        # CAP_DSHOW is fast and reliable on Windows
        ...
        if cap.isOpened():
            ...
            cap.release()
            if ret:
                cam_label = "Logi C270 HD WebCam" if (idx == 2 or w >= 1280) else f"Camera {idx}"
                logger.info(f"detect_camera_index: Selected {cam_label} at index {idx} ({w}x{h})")
                return idx
        else:
            cap.release()
            ...
            if cap2.isOpened():
                ...
                cap2.release()
                if ret:
                    logger.info(f"detect_camera_index: found camera (default backend) at index {idx}")
                    return idx
            cap2.release()
    logger.warning("detect_camera_index: no working camera found in range 0..%d", max_index)
    return None
...
class CameraCapture:
    ...
    def start(self) -> None:
        ...
        if self.is_video_file:
            ...
            # Try DSHOW first (Windows DirectShow — fastest, most reliable)
            self._cap = cv2.VideoCapture(dev_idx, cv2.CAP_DSHOW)
            if not self._cap.isOpened():
                self._cap = cv2.VideoCapture(dev_idx)

            # If the requested index failed, auto-scan for any working camera
            if not self._cap.isOpened():
                logger.warning(
                    f"Camera index {dev_idx} not available — auto-scanning for any working camera..."
                )
                found_idx = detect_camera_index(skip=[dev_idx])
                if found_idx is not None:
                    logger.info(f"Auto-detected camera at index {found_idx}, switching.")
                    self.source = found_idx
                    dev_idx = found_idx
                    self._cap = cv2.VideoCapture(dev_idx, cv2.CAP_DSHOW)
                    if not self._cap.isOpened():
                        self._cap = cv2.VideoCapture(dev_idx)
            ...
            if not self._cap.isOpened():
                raise RuntimeError(
                    f"Cannot open any camera. Tried index {dev_idx}. "
                    "Check that a USB camera is plugged in and not in use by another app."
                )
