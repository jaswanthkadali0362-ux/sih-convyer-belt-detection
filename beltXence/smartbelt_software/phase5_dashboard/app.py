...
# 4. Hardware Interface & COM Ports
st.sidebar.subheader("4. Hardware Interface & COM Ports")

# Scan available host COM ports
import serial.tools.list_ports
detected_phase5_ports = list(serial.tools.list_ports.comports())

c_rescan1, c_rescan2 = st.sidebar.columns([3, 2])
with c_rescan1:
    if st.button("🔄 Rescan Ports", key="p5_rescan", use_container_width=True):
        st.rerun()
with c_rescan2:
    st.caption(f"{len(detected_phase5_ports)} found")

p5_port_options = ["Auto-Detect (ESP32 / Arduino)"]
for p in detected_phase5_ports:
    p5_port_options.append(f"{p.device} — {p.description}")
p5_port_options.append("🧪 Simulation Mode (Virtual Telemetry)")
p5_port_options.append("Custom Port Name...")

selected_p5_port = st.sidebar.selectbox(
    "Active Hardware COM Port",
    options=p5_port_options,
    index=1 if detected_phase5_ports else 0,
)

if selected_p5_port == "Custom Port Name...":
    custom_p5 = st.sidebar.text_input("Custom Port", value="COM19")

p5_baud = st.sidebar.selectbox("Baud Rate", [115200, 9600, 57600, 38400], index=0)

if detected_phase5_ports:
    first_p = detected_phase5_ports[0].device
    st.sidebar.markdown(
        f"""
        <div class="hardware-pill-sim" style="border: 1px solid #28a745; color: #4ade80;">
            ● SERIAL BUS: DETECTED ({len(detected_phase5_ports)} Port{'s' if len(detected_phase5_ports) != 1 else ''})<br>
            Selected: <b>{selected_p5_port.split(' — ')[0]}</b><br>
            Baud: {p5_baud} bps
        </div>
        """,
        unsafe_allow_html=True,
    ...
    st.sidebar.markdown(
        ...
        <div class="hardware-pill-sim">
            ...
            No physical COM ports detected<br>
            Baud: {p5_baud} (Virtual Mode)
        </div>
        """,
        unsafe_allow_html=True,
    )
